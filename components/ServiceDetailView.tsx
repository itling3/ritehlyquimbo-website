...
const ServiceDetailView: React.FC<ServiceDetailViewProps> = ({ service, onBack, onBook, onPricingClick }) => {
  return (
    <div className="pt-4 md:pt-6 pb-16 md:pb-24 px-4 md:px-6 animate-in fade-in duration-500">
      <SEO 
        title={service.seoTitle || `${service.title} | ${service.features[0] || 'Scale Your Results'}`} 
      ...
      <div className="max-w-5xl mx-auto">
        <div className="grid lg:grid-cols-3 gap-8 md:gap-12 items-start mb-12 md:mb-20">
          ...
          <div className="flex flex-col gap-6 md:gap-8 lg:sticky lg:top-32">
            {service.landmarkImage && (
              <div className="relative group overflow-hidden rounded-2xl md:rounded-3xl border border-white/10 shadow-2xl">
                <img 
                  src={service.landmarkImage} 
                  alt={`${service.title} landmark`} 
                  className="w-full h-48 md:h-64 object-cover transform transition-transform duration-700 group-hover:scale-110"
